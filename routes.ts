...
export async function handleApi(req: Request, url: URL): Promise<Response> {
  ...
  if (path === '/health') return json({ status: 'ok', service: 'Pizzeria Vocal SaaS Deno', version: '3.0.0', timestamp: new Date().toISOString() });
  ...
  if (path === '/dashboard/overview' && method === 'GET') {
    try {
      ...
      return json({ restaurants: restaurants.count??0, calls_today: callsToday.count??0, calls_total: callsTotal.count??0, orders_today: ordersToday.data?.length??0, revenue_today: revenueToday.toFixed(2), revenue_month: revenueMonth.toFixed(2), avg_basket: ordersToday.data?.length?(revenueToday/ordersToday.data.length).toFixed(2):'0', customers_total: customersTotal.count??0, conversion_rate: callsToday.count?Math.round(((ordersToday.data?.length??0)/callsToday.count)*100):0 });
  ...
  if (path === '/restaurants' && method === 'POST') {
    ...
    const {data,error} = await db.from('restaurants').insert({name,type:type||'pizza',address,phone,owner_phone,owner_email,config_json:config_json||{}}).select().single();
    if (error) return json({error:error.message},500);
    return json(data,201);
  }

  const rMatch = path.match(/^\/restaurants\/([^/]+)$/);
  if (rMatch) {
    const id = rMatch[1];
    if (method==='GET') { const {data,error}=await db.from('restaurants').select('*').eq('id',id).single(); if(error)return json({error:'Restaurant introuvable'},404); return json(data); }
    if (method==='PATCH') { const body=await parseBody(req); const {data,error}=await db.from('restaurants').update({...body,updated_at:new Date().toISOString()}).eq('id',id).select().single(); if(error)return json({error:error.message},500); return json(data); }
  }

  const delayMatch = path.match(/^\/restaurants\/([^/]+)\/delay$/);
  if (delayMatch && method==='PUT') {
    const id=delayMatch[1]; const body=await parseBody(req); const delay_minutes=body.delay_minutes as number;
    if (typeof delay_minutes!=='number'||delay_minutes<5||delay_minutes>120) return json({error:'delay_minutes entre 5 et 120'},400);
    const {data:cur}=await db.from('restaurants').select('config_json').eq('id',id).single();
    await db.from('restaurants').update({config_json:{...(cur?.config_json??{}),delay_minutes}}).eq('id',id);
    return json({success:true,delay_minutes});
  }

  const statsMatch = path.match(/^\/restaurants\/([^/]+)\/stats$/);
  if (statsMatch && method==='GET') {
    const id=statsMatch[1]; const today=new Date(); today.setHours(0,0,0,0);
    const [calls,orders]=await Promise.all([db.from('calls').select('id',{count:'exact',head:true}).eq('restaurant_id',id).gte('started_at',today.toISOString()),db.from('orders').select('total').eq('restaurant_id',id).gte('created_at',today.toISOString())]);
  ...
  if (path==='/calls' && method==='GET') {
    const limit=parseInt(url.searchParams.get('limit')||'50'); const restaurantId=url.searchParams.get('restaurant_id');
    let query=db.from('calls').select('*, restaurants(name, type)').order('started_at',{ascending:false}).limit(limit);
    if (restaurantId) query=query.eq('restaurant_id',restaurantId);
    const {data,error}=await query; if(error)return json({error:error.message},500); return json(data??[]);
  }

  if (path==='/orders' && method==='GET') {
    const limit=parseInt(url.searchParams.get('limit')||'50'); const restaurantId=url.searchParams.get('restaurant_id'); const period=url.searchParams.get('period');
  ...
  if (path==='/orders/revenue' && method==='GET') {
    ...
    const since=period==='week'?new Date(Date.now()-7*86400000):period==='year'?new Date(Date.now()-365*86400000):new Date(Date.now()-30*86400000);
    const {data}=await db.from('orders').select('created_at, total').gte('created_at',since.toISOString()).order('created_at');
    const byDay:Record<string,number>={};
    for(const o of data??[]){const d=o.created_at.split('T')[0];byDay[d]=(byDay[d]||0)+parseFloat(o.total??0);}
  ...
  if (path==='/relances/campaign' && method==='POST') {
    try {
      ...
      for(const c of customers){
        const msg=(message as string).replace('{prenom}',c.first_name||'cher client');
        const r=await sendMarketingSms({clientPhone:c.phone,message:msg,promoCode:promo_code as string});
        if(r.success){sent++;await db.from('sms_relances').insert({restaurant_id,customer_id:c.id,phone_to:c.phone,message:msg,promo_code,discount_pct,status:'sent',twilio_sid:r.sid,sent_at:new Date().toISOString(),campaign_name,trigger_type:trigger_type||'manual'});}
      }
  ...
  if (path==='/saas/revenue' && method==='GET') {
    ...
    return json({mrr:mrr.toFixed(2),restaurants_total:restaurants?.length??0,restaurants_active:active.length,arr:(mrr*12).toFixed(2),avg_revenue_per_restaurant:active.length?(mrr/active.length).toFixed(2):'0'});
  }

  if (path==='/check-retell' && method==='GET') {
    if(url.searchParams.get('secret')!=='PIZZERIA_SETUP_2024')return json({error:'Forbidden'},403);
    try { const res=await fetch(`https://api.retellai.com/v2/phone-number/+33189480917`,{headers:{'Authorization':`Bearer ${config.retell.apiKey}`}}); return json({success:true,retell_data:await res.json()}); }
    catch(err){return json({error:String(err)},500);}
  }
